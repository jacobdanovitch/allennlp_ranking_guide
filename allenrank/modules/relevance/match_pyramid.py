...
@RelevanceMatcher.register('matchpyramid')
class MatchPyramid(RelevanceMatcher):
    '''
    ...
    * somewhat-third-hand reference: https://github.com/NTMC-Community/MatchZoo/blob/master/matchzoo/models/matchpyramid.py
    ...
    def __init__(self,
        ...
        for i in range(len(conv_output_size)):
            conv_layer_dict["pad_" +str(i)] = nn.ConstantPad2d((0,conv_kernel_size[i][0] - 1,0, conv_kernel_size[i][1] - 1), 0)
            conv_layer_dict["conv_"+str(i)] = nn.Conv2d(kernel_size=conv_kernel_size[i], in_channels=last_channel_out, out_channels=conv_output_size[i])
            conv_layer_dict["relu_"+str(i)] = nn.ReLU()
            conv_layer_dict["pool_"+str(i)] = nn.AdaptiveMaxPool2d(adaptive_pooling_size[i])
